...
const Product = ({ product }) => {
  ...
  const handleAddToCart = async () => {
    const res = await axios.get(`/api/product/${product._id}`);
      ...
      <Link href={`/product/${product.slug}`}>
        ...
        <div className="py-3 text-start relative">
          ...
          <div className="absolute top-2 right-1 w-[min-content] flex items-center divide-dotted divide-x-2 divide-[#d7f4db] border-[#cce7d0] border-2 rounded-lg">
            <span className="px-2 bg-[#e8f6ea] text-[#088178] flex items-center gap-1">
              {product.rating}
              <AiTwotoneStar size={25} color="#FFD700" />
            </span>
            <span className="px-2">{product.numReviews}</span>
          </div>
          <h4 className="text-[#088178] font-semibold text-lg">
            ${product.price}
          </h4>
        </div>
      </Link>
      <div onClick={handleAddToCart} className="cartIcon">
        <FaShoppingCart size={25} color="#088178" />
      </div>
    </div>
  );
};
