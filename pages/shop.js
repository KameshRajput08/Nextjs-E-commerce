    ...
    return (
        <Layout title={`Shop-${search}`}>
            <div className='px-10 py-4 flex items-center flex-col justify-between lg:flex-row'>
                <h3 className='text-2xl font-semibold my-4'>{search == "" ? 'Home/Shop' : `Results for ${search}`}</h3>
                <div className='flex flex-col lg:flex-row gap-4 justify-between items-center'>
                    <div className='flex'>
                        <span className="font-semibold hidden lg:block text-lg mr-4">Sort Products:</span>
                        <select className='outline-none p-[5px] mr-2 lg:mr-5 bg-[#e3e6f3] border-2 border-[#333333] opacity-70 rounded-lg' onClick={(e) => setSort(e.target.value)}>
            ...
            <div className='px-10 py-5 pb-10 flex justify-center flex-wrap gap-10'>
                {
                    products === [] ? <div>Products not found</div> : products?.map(p => {
